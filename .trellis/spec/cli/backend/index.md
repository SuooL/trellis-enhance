...
| [Release Process](./release-process.md) | No-npm release model: `dev` → `main` promotion PR + `v<version>` tag, package versioning, manifest continuity, submodule ordering | Done |
...
| [`trellis upgrade` Command](./commands-upgrade.md) | Self-hosted upgrade guidance: prints `git pull` + rebuild steps, no package manager invoked | Done |
...
- Cutting a release (promotion PR + tag; this fork never publishes to npm) / submodule coordination / manifest continuity → [release-process.md](./release-process.md)
