...
| Symptom | Cause | Fix |
|---|---|---|
| `trellis: command not found` | CLI not linked globally | re-link the source clone: `cd <trellis-enhance clone>/packages/cli && pnpm link --global` (this CLI is not on npm) |
