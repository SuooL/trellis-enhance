...
Do not assume the user has the Trellis source repository, and do not default to modifying wherever the CLI itself is installed — solve the user's need inside their project's own files. The CLI is distributed as a source clone symlinked onto `PATH` (not an npm package), so editing it in place would silently patch the user's global tool and be overwritten on their next rebuild.
...
- `references/local-architecture/workflow.md`: Phases, routing, workflow-state blocks, and selectable workflow templates (bundled `native`, plus whatever the configured marketplace serves) in `.trellis/workflow.md`.
...
- `.trellis/workflow.md` is the local workflow source of truth; its initial content was selected from a workflow template (bundled `native`, or a marketplace template) at `trellis init` time and can be re-selected via `trellis workflow --template <id>`. `native` is the only id the CLI ships; every other id must be served by the configured marketplace, so `trellis workflow --list` is the authoritative list. Missing `.trellis/agents/<name>.md` files referenced by the active template trigger a non-blocking stderr warning pointing at `trellis update`.
...
- Do not modify the CLI's own install location — the source clone it is symlinked from, or any `node_modules` copy of it — to implement project needs; that patches the user's global tool and is lost on their next rebuild.
