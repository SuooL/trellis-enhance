...
(`mindfold-ai/Trellis`). The package is renamed to **`trellis-enhance`** (bins stay `trellis` / `tl`) and is
**never published to any npm registry** — install is source-clone + build + global symlink (see below);
...
- **`upstream`** remote = `mindfold-ai/trellis` — kept **only** for occasional manual comparison: `git fetch upstream && git diff upstream/main main -- <paths>`, cherry-pick what's worthwhile. Not auto-tracked/merged. (This repo is standalone, no longer a GitHub fork.)
  - ⚠️ **One-time per clone, before the first `git fetch upstream`:** `git config remote.upstream.tagOpt --no-tags`. Tags live in `.git/config`, which is not version-controlled, so this does **not** travel with a clone. Without it, `git fetch upstream` drags back the 33 upstream tags that were deliberately pruned (they aren't on `main`'s ancestor chain and make `git tag` / release tooling meaningless). This is separate from the 103 tags that legitimately remain — those *are* all reachable from `main`, and `release.js` pushes only the one new tag per release, never `--tags`. Verify with `git config --get remote.upstream.tagOpt` → `--no-tags`.
...
| **Git/CI standard** (always generated) | 3-branch model (main=release / dev=integration+CD-to-prod / `feature/<task-slug>`); PR→dev via `task.py create-pr` (it owns the push) → CI → auto-merge with `--delete-branch`; dev→prod SSH deploy (opt-in via `vars.DEPLOY_ENABLED`); manual `workflow_dispatch` release; weekly branch prune. Emitted on every `trellis init`. **Generated-CI hard gates = build + tests + diff-coverage ≥ 80% only** — the template's `verify` job has no typecheck/lint step, and `git-workflow.md` states lint / type-check are informational, not merge gates. (THIS repo's own `.github/workflows/ci.yml` is stricter: typecheck + lint are hard gates there too. Don't conflate the two.) | `templates/git-workflow/` (spec + 4 `.github/workflows/*.yml` + README), wired in `templates/git-workflow/index.ts` + `configurators/workflow.ts` + `templates/trellis/index.ts` |
